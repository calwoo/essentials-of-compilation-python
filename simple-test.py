...
if __name__ == "__main__":
    ...
    for program_filename in tests:
        ...
        print(repr(program_ast))
        interp_and_check(
            program_filename, program_ast, interp_Lvar, "remove complex operands"
        )
        print()
        ...
        if False:
            ...
            interp_and_check(
                program_filename, program_ast, interp_x86, "select instructions"
            )

            # Assign Homes
            program_ast = C.assign_homes(program_ast)
            interp_and_check(program_filename, program_ast, interp_x86, "assign homes")

            # Patch Instruction
            program_ast = C.patch_instructions(program_ast)
            interp_and_check(
                program_filename, program_ast, interp_x86, "patch instructions"
            )
