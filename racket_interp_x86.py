...
def convert_def(df):
    match df:
        case utils.FunctionDef(label, [], body, _, rtyp, _):
            ...
            return (
                f"(Def '{label_name_(label)} '() '{convert_type(rtyp)}"
...
def convert_value(v):
    if isinstance(v, dict):
        pairs = [
            f"({convert_value(key)} . {convert_value(value)})"
...
def convert_info(node):
    ...
    c_fields = {
        ...
        "type_comment",
        "type_params",
    }  # set([f.name for f in fields(node)])
    info_fields = all_fields - c_fields
    pairs = [
        f"({info_field.replace('_', '-')} . "
